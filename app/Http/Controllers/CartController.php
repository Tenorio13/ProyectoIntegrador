...
class CartController extends Controller
{
    public function update(Request $req){
        ...
        $currentCart->precio         = $price;
        $currentCart->cantidad         = $cantidad;

        $currentCart->total         = $total;

        $currentCart->order_date    = date('d-m-Y');
        $currentCart->status        = "Pendiente";
        $currentCart->arrived_date   = "Activo";
    ...
    public function updateStatus(Request $req){
        ...
        $notification           = "¡El status   ha cambiado!";
        return back()->with(compact('notification'));
    }
    ...
    public function destroy2( $ventas){
        $compras= Cart::find( $ventas);
        $compras->delete( $ventas);
        $notification = "¡La compra se cancelo   correctamente!.";
        return back()->with(compact('notification'));
    }
